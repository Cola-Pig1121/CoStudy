...
export function Sidebar() {
  ...
  return (
    <aside
      className={cn(
        "fixed left-0 top-0 bottom-0 bg-white border-r border-gray-100 flex flex-col z-40 transition-all duration-300",
        width
      )}
    >
      {/* Logo + 折叠按钮 */}
            ...
            if (item.disabled) {
              return (
                <div
                  key={item.href}
                  className={cn(
                    "flex items-center gap-3 rounded-xl text-sm text-gray-300 cursor-not-allowed",
      ...
      <div className="border-t border-gray-100 p-2 shrink-0">
        ...
        <div
          className={cn(
            "flex items-center gap-3 mt-1 rounded-xl bg-gray-50",
